...
namespace caret {
    ...
    struct VoxelIJK
    {
        int64_t m_ijk[4];
        VoxelIJK() { }
        VoxelIJK(int64_t i, int64_t j, int64_t k) { m_ijk[0] = i; m_ijk[1] = j; m_ijk[2] = k;  m_ijk[3] = 0; }
        VoxelIJK(int64_t i, int64_t j, int64_t k, int64_t t) { m_ijk[0] = i; m_ijk[1] = j; m_ijk[2] = k;  m_ijk[3] = t; }
        template<typename T>
        VoxelIJK(const T ijk[4]) {
            m_ijk[0] = ijk[0];
            m_ijk[1] = ijk[1];
            m_ijk[2] = ijk[2];
            m_ijk[3] = ijk[3];
        ...
        {
            if (m_ijk[3] < rhs.m_ijk[3]) return true;//compare such that when sorted, m_ijk[0] moves fastest
            if (m_ijk[3] > rhs.m_ijk[3]) return false;
